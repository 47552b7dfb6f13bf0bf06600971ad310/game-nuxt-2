  ...
  <div class="bg-card rounded-xl">
    ...
    <div v-if="!!user && !loading">
      ...
      <UiFlex
        v-if="user.currency"
        :class="[
          'divide-x dark:divide-gray-900',
          'border-t dark:border-gray-900',
        ]"
      >
        <UiFlex type="col" class="p-2 w-4/12">
          <UiText size="xs" color="gray" class="mb-1">Xu Web</UiText>
          <UiText weight="bold" @click="useTo().navigateToSSL('/payment'), emit('action')" pointer>{{ miniMoney(user.currency?.coin) }}</UiText>
        </UiFlex>
        ...
        <UiFlex type="col" class="p-2 w-4/12">
          ...
          <UiText weight="bold" @click="useTo().navigateToSSL('/minigame/wheel'), emit('action')" pointer>{{ miniMoney(user.currency?.wheel) }}</UiText>
        </UiFlex>
      </UiFlex>
