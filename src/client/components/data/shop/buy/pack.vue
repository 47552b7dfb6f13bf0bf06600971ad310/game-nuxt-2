<template>
  <UiContent no-dot title="Cửa Hàng Vật Phẩm" sub="Mua vật phẩm vào trong game" class="bg-card rounded-xl p-4">
...
const submit = async () => {
  try {
    buying.value = true
    const data = await useAPI('shop/public/pack/buy', JSON.parse(JSON.stringify(state.value)))
