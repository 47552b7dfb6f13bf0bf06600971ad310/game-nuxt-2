...
const validate = (state) => {
  ...
  if (!state.confirm_password) errors.push({ path: 'confirm_password', message: 'Vui lòng nhập đầy đủ' })
...
const submit = async () => {
  try {
    ...
    await useAPI('auth/public/sign/landing/up', JSON.parse(JSON.stringify(state.value)))
