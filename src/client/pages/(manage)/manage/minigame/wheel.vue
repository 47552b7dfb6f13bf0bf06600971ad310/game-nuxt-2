<template>
  <UiContent title="Lucky Wheel" sub="Quản lý Vòng quay may mắn">
    <UiFlex class="gap-1">
...
const getList = async () => {
  try {
    ...
    const data = await useAPI('minigame/wheel/manage/list', JSON.parse(JSON.stringify(page.value)))
...
const addAction = async () => {
  try {
    ...
    await useAPI('minigame/wheel/manage/add', JSON.parse(JSON.stringify(stateAdd.value)))
...
const editAction = async () => {
  try {
    ...
    await useAPI('minigame/wheel/manage/edit', JSON.parse(JSON.stringify(stateEdit.value)))
