    ...
    <UCard class="my-2" :ui="{ body: { padding: 'p-0 sm:p-0' } }">
      ...
      >
        <template #url-data="{ row }">
          <NuxtLink target="_blank" :to="`/ads/landing/${row.code}`">{{ useMakeLink().link(`/ads/landing/${row.code}`) }}</NuxtLink>
        </template>
...
const getList = async () => {
  try {
    ...
    const data = await useAPI('ads/landing/manage/list', JSON.parse(JSON.stringify(page.value)))
...
const addAction = async () => {
  try {
    ...
    await useAPI('ads/landing/manage/add', JSON.parse(JSON.stringify(stateAdd.value)))
...
const editAction = async () => {
  try {
    ...
    await useAPI('ads/landing/manage/edit', JSON.parse(JSON.stringify(stateEdit.value)))
