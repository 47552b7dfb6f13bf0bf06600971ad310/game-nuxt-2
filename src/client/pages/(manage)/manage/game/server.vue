<template>
  <UiContent title="Server" sub="Thông tin máy chủ trò chơi" class="max-w-[700px] mx-auto">
    <UiFlex class="gap-1">
      ...
      <SelectDate v-model="page.range.end" placeholder="Kết thúc" size="sm" class="max-w-[140px] mr-auto"/>
...
const getList = async () => {
  try {
    ...
    const data = await useAPI('game/manage/server', JSON.parse(JSON.stringify(page.value)))
